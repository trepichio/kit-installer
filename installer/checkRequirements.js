...
module.exports = async () => {
  ...
  logger.info(
    `checkRequirements: mustInstall ${JSON.stringify([...mustInstall])}`
  );
      ...
      for (const installedProgram of installedPrograms) {
        if (
          ...
          compareVersions.compare(
            reqProgram.DisplayVersion,
            installedProgram.DisplayVersion,
            reqProgram.comparatorVersion
          )
        ) {
  ...
  async function setPath(program) {
    if (!shell.env.PATH.includes(program)) {
      shell.echo(`added ${program} to ENV PATH`);
      shell.exec(
        `powershell -NoExit -Command "setx /M PATH '${shell.env.PATH};${program}';exit;"`
      );
      logger.log(
        "info",
        "shell path modified: %s",
        (shell.env.PATH = `${shell.env.PATH};${program}`)
      );
    } else {
      logger.info("No changes made to ENV PATH.");
